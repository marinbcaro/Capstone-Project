...
public class InMemoryProductsRepository implements ProductsRepository {
    ...
    @Override
    public void getProducts(String catdId,@NonNull final ProductsRepository.LoadProductsCallback callback) {
        checkNotNull(callback);
        // Load from API only if needed.
        if (mCachedCategories == null) {
            mCategoriesServiceApi.getProductsCategories(catdId,new ProductsServiceApi.ProductsServiceCallback<List<Product>>() {
                @Override
